...
import { loadLatestSnapshotFromFolders } from "../migration-loader.js";
import * as p from "@clack/prompts";
import pc from "picocolors";

// ---------------------------------------------------------------------------
// generate — diff-based SQL migration generator
// Generates a migration directory containing:
//   - up.sql
//   - down.sql
//   - snapshot.json
// ---------------------------------------------------------------------------
...
export async function runGenerate(
  ...
  const prevSnapshot = (await loadLatestSnapshotFromFolders(migrationsDir)) || { tables: {}, enums: {}, views: {} };
  const isFirstMigration = Object.keys(prevSnapshot.tables).length === 0 &&
    Object.keys(prevSnapshot.enums).length === 0 &&
    Object.keys(prevSnapshot.views).length === 0;
  ...
  s.start("Writing migration files...");

  // ── Write migration directory & files ─────────────────────────────────────
  await Bun.$`mkdir -p ${migrationFolder}`.quiet();

  const upSqlPath = join(migrationFolder, "up.sql");
  const downSqlPath = join(migrationFolder, "down.sql");
  const snapshotPath = join(migrationFolder, "snapshot.json");

  await Bun.write(upSqlPath, upStatements.join("\n") + "\n");
  await Bun.write(downSqlPath, downStatements.join("\n") + "\n");
  await Bun.write(snapshotPath, JSON.stringify(currentSnapshot, null, 2) + "\n");

  s.stop(`Generated migration ${pc.cyan(folderName)}`);
